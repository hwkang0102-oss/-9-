...
module RISC_TOY (
    ...
    // 🌟 [추가됨] 동기식 메모리의 1클럭 딜레이를 방어하는 2-Cycle Flush 로직
    reg         Flush_Delay;
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            Flush_Delay <= 1'b0;
        end else begin
            Flush_Delay <= Branch_Taken; // 브랜치 발생 사실을 1클럭 기억해둠
        end
    end
    
    // Extended_Flush: 점프 당장 발생 시점(1) + 메모리가 뒤늦게 뱉는 시점(1) = 총 2클럭 동안 차단막 생성
    ...
    wire [31:0] safe_PC    = Was_Stalled ? (PC - 32'd4) : PC;
    ...
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            IF_ID_PC   <= 32'd0;
            IF_ID_Inst <= 32'd0;
        end else if (Extended_Flush) begin   // 🌟 [수정됨] 2클럭짜리 철벽 방패 적용!
            // Branch taken → 메모리 딜레이로 인해 뒤늦게 들어오는 악성 명령어 완벽 차단 (NOP)
            IF_ID_PC   <= 32'd0;
            IF_ID_Inst <= 32'd0;
        end else if (Stall_Flag) begin
