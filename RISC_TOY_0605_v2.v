...
module RISC_TOY (
    ...
    // =========================================================================
    wire        Extended_Flush = Branch_Taken;   // 1클럭 flush
    ...
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            ...
            // ★ 점프가 일어나도 IF/ID 입구는 NOP 으로 막지 않는다.
            //   - Branch_Taken 클럭에 죽여야 할 명령은 "이미 IF/ID 안에 있는" 명령이고,
            //     그건 ID/EX 입구의 Branch_Taken flush 가 NOP 으로 만든다([4] 참조).
            //   - IF/ID 입구를 막으면 "다음 클럭에 올라올 점프 목적지"가 죽어버린다.
            //   따라서 IF/ID 는 메모리에서 올라온 명령을 항상 그대로 싣는다.
            IF_ID_PC    <= safe_PC;
    ...
    assign Mem_Stall = EX_MEM_Ctrl_MemRead_r & ~Mem_Stall_done;
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
