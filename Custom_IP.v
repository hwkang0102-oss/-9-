...
module CUSTOM_IP (
    ...
    output reg         IP_VALID // [NEW] 호스트 CPU에게 데이터 준비 완료를 알리는 플래그
);
    ...
    // 공간 연산에 필요한 총 15개의 16비트 고정 소수점 변수들을 저장할 임시 저장 플립플롭 배열 선언 
    // [FIX] 공간 좌표는 음수가 될 수 있으므로 모두 signed 선언 추가!
    ...
    reg signed [15:0] cache_dir_x, cache_dir_y, cache_dir_z; // 바로 직전에 계산했던 광선의 3차원 방향 벡터 값을 보관
    ...
    // 삼항 연산자(? :)를 써서 "더 큰 값에서 작은 값을 빼라"는 크기 비교기(Comparator)와 감산기(Subtractor) 3세트
    ...
    // X, Y, Z 세 축의 오차값(diff)이 모두 호스트 CPU가 설정해 준 임계값(epsilon_th)보다 작으면서, 캐시가 비어있지 않을 때(cache_dir_x != 0), is_coherent 와이어 선에 1을 보냄.
    wire is_coherent = (diff_x < epsilon_th) && (diff_y < epsilon_th) && (diff_z < epsilon_th) && (cache_dir_x != 16'd0); 
    
    // 데이터 로드가 끝났고, 일관성이 깨졌을 때만 무거운 메인 코어 엔진을 가동 (Clock Gating)
    // 데이터 수신은 완벽히 끝났는데(parsing_done = 1), 오차가 임계값을 넘어서 일관성이 깨졌을 때만(is_coherent = 0 $\rightarrow$ !is_coherent = 1) 최종적으로 enable_core_pipeline = 1 신호를 킴
    ...
    // =========================================================================
    // [FIX] 코어 파이프라인의 데이터 흐름을 추적하는 Valid 시프트 레지스터 추가
    reg [2:0] pipe_valid;

    // 파이프라인 스테이지 1: 하드웨어 승산기를 이용한 t 연산 (조합 회로 무제한 병렬화)
    // 6채널 완전 병렬 공간 교차 연산 엔진
    // 레이 트레이싱의 근간 수식인 아래의 거리 연산 공식을 하드웨어화 (t = (Box - Ray_{orig}) \times Ray_{inv})
    // X, Y, Z 세 개 축의 최소 경계면(box_min)과 최대 경계면(box_max)에 대해 6개의 하드웨어 곱셈기(Multiplier) 소자를 칩 안에 동시에 병렬로 배치
    // 전기 신호가 인입되자마자 6개 수식의 정답 와이어(raw_t) 6줄이 단 1클록 만에 동시에 튀어나옴 (수정본에서는 파이프라인 레지스터로 분리)
    ...
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            ...
            if (enable_core_pipeline) begin //앞서 계산한 클록 게이팅 신호(enable_core_pipeline)가 1일 때만 이 레지스터들이 작동
    ...
    // 6채널 곱셈기에서 튀어 나온 생생한 원시 데이터들을 캡처해서 안전하게 한 박스 킵해둘 파이프라인 스테이지 2 물리 레지스터 공간 선언
    ...
    always @(posedge CLK or negedge RSTN) begin // 리셋 신호 유입 시 곱셈 데이터 6축 보관소 레지스터들을 깨끗하게 0으로 초기화
        if (!RSTN) begin
            ...
            if (pipe_valid[0]) begin
                ...
                // 광선이 거꾸로 날아와서 최소/최대 거리 부호가 뒤집히는 극단적인 상황(Corner Case)에 대비하여, 내부 멀티플렉서 스위치를 통해 자동으로 더 작은 값을 tmin방에, 더 큰 값을 tmax방에 정렬시켜 저장
                // [23:8]이라는 비트 슬라이싱 테크닉을 적용해, 32비트로 뻥튀기된 곱셈 결과 데이터 중 소수점 위치에 맞는 유효 데이터 대역 16비트만 칼 같이 뽑아내어 저장하는 고정 소수점 스케일 다운 가속 최적화를 동시에 수행
                r_tmin_x <= (stg1_tx1 > stg1_tx2) ? stg1_tx2[23:8] : stg1_tx1[23:8]; 
                r_tmax_x <= (stg1_tx1 > stg1_tx2) ? stg1_tx1[23:8] : stg1_tx2[23:8]; 
                
                // [23:8]인 이유는 사용하는 고정 소수점(Fixed-point) 포맷이 소수부(Fractional bits)로 8비트를 사용한다고 가정했기 때문. 16bits x 16bits 곱셈을 하면 소수부가 16비트로 늘어나므로, 이를 다시 원래의 8비트 정밀도로 맞추기 위해 하위 8비트를 버리고 그 위부터 16비트([23:8])를 취하는 고정 소수점 스케일링 복원 연산
                r_tmin_y <= (stg1_ty1 > stg1_ty2) ? stg1_ty2[23:8] : stg1_ty1[23:8]; 
                r_tmax_y <= (stg1_ty1 > stg1_ty2) ? stg1_ty1[23:8] : stg1_ty2[23:8];
                r_tmin_z <= (stg1_tz1 > stg1_tz2) ? stg1_tz2[23:8] : stg1_tz1[23:8];
                r_tmax_z <= (stg1_tz1 > stg1_tz2) ? stg1_tz1[23:8] : stg1_tz2[23:8];
            end
        end
    end
    ...
    // 3D 레이 트레이싱 수학 원리에 따라, 최종 진입점은 각 축의 최소 거리 결과 중 최대값(Max of Mins)을 구하고, 최종 이탈점은 각 축의 최대 거리 결과 중 최소값(Min of Maxes)을 구해야함.
    // 코드 그대로 x와 y를 먼저 대결시켜 승자를 뽑고(inter), 그 승자와 나머지 z축 데이터를 최종 대결시키는 2단계 하드웨어 토너먼트 트리로 설계
    ...
    // pipeline_hit: 상자 이탈 지점(t_{max\_final})이 진입 지점$t_{min\_final})보다 멀거나 같으면서 양수 대역에 존재할 때, 광선이 상자를 완벽히 관통했다는  플래그 신호 1을 생성
    wire        pipeline_hit = (t_max_final >= t_min_final) && (t_max_final > 16'sd0); 
    
    // pipeline_computed_result:  맨 왼쪽 31번 비트에는 방금 구한 Hit 플래그(1비트)를 싣고, 중간 15비트는 0으로 채운 뒤, 하위 16비트에는 최종 충돌 거리 정답인 t_min_final을 묶어 총 32비트 규격의 단일 데이터 패킷 패킹을 완성
    wire [31:0] pipeline_computed_result = { pipeline_hit, 15'd0, t_min_final }; 

    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) pipe_valid[2] <= 1'b0;
        else       pipe_valid[2] <= pipe_valid[1];
    end
    ...
    always @(posedge CLK or negedge RSTN) begin // 리셋 유입 시 가속기 출력 포트(IPOUT)와 내부 캐시 금고들을 깔끔하게 초기 조건인 0으로 초기화
        if (!RSTN) begin
            ...
            // 스마트 바이패스(Smart Bypass) 데이터 포워딩 멀티플렉서 스위치
            if (parsing_done && is_coherent) begin
                ...
                // 오차가 커서 바이패스를 못 할 때는 파이프라인 엔진이 계산해 낸 pipeline_computed_result 결과를 IPOUT으로 보냄. 동시에 다음번에 날아올 광선이 현재와 비슷한 궤적인지 비교할 수 있도록, 현재 광선의 3축 방향 벡터(ray_dir)와 이번에 구한 정답 패킷을 캐시 금고 레지스터 세트에 갱신
                ...
                // 데이터 수집 중(parsing_done = 0)이거나 연산이 완전히 끝나서 CPU가 명령을 해제했을 때(ip_start = 0), 시스템 버스에 쓰레기 값이 흘러 다니지 않도록 IPOUT을 0으로 청소(Flush)하여 공유 버스를 안정화하는 하드웨어 매너 회로
                // 가속 유닛 제어 해제 시 시스템 버스 플러시(Flush)
                IPOUT <= 32'd0;
            end
        end
    end
