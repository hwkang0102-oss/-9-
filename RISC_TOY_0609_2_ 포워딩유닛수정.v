...
module RISC_TOY (
    ...
    // =========================================================================
    // [FIX] Register-File Write-Forwarding (WB -> ID 내부 바이패스)
    //   문제 : REGFILE(model.v)은 동기 쓰기 + #1 지연 비동기 읽기 구조라,
    //          어떤 명령이 ID 단계에서 읽는 레지스터를, 같은 사이클의 WB 단계가
    //          기록하면 그 갱신이 읽기에 반영되지 못해 옛값을 ID/EX 에 래치한다.
    //          EX 단계 포워딩은 EX/MEM, MEM/WB(거리 1~2)만 커버하므로, 생산자가
    //          소비자의 EX 도달 직전에 MEM/WB 밖으로 은퇴하는 "거리 3" RAW
    //          해저드에서 포워딩이 사라져 깨진다.
    //   해결 : 레지스터파일을 쓰기-우선(write-first)처럼 동작시켜, WB 가 지금
    //          기록 중인 레지스터를 ID 가 읽으면 갓 쓴 WB_data 를 바로 넘겨준다.
    wire [31:0] Rs1_ID_fwd = (WB_RegWrite && (WB_Rd != 5'd0) && (WB_Rd == Read_Addr_0))
                             ? WB_data : Rs1_Data_out;
    wire [31:0] Rs2_ID_fwd = (WB_RegWrite && (WB_Rd != 5'd0) && (WB_Rd == Read_Addr_1))
                             ? WB_data : Rs2_Data_out;
    ...
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            ...
            ID_EX_PC        <= IF_ID_PC;
            ID_EX_Rs1_data  <= Rs1_ID_fwd;
            ID_EX_Rs2_data  <= Rs2_ID_fwd;
