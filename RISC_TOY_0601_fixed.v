...
module RISC_TOY (
    ...
    // =========================================================================
    // 동기식 명령어 메모리(INST_RAM: latch 1클럭 + #ATIME 지연)로 인해,
    // 점프로 죽여야 할 명령어가 IF/ID 에 "늦게" 도착한다.
    // Branch_Taken 시점만 기억하는 게 아니라, 그 잔향을 여러 클럭 추적해서
    // 그 구간에 IF/ID 로 들어오는 명령을 모두 "무효(valid=0)"로 태깅한다.
    // (Flush_Delay 2단 = 점프 결정 + 이후 2클럭, 총 3클럭 추적)
    // =========================================================================
    reg         Flush_Delay;     // T+1
    reg         Flush_Delay2;    // T+2
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            Flush_Delay  <= 1'b0;
            Flush_Delay2 <= 1'b0;
        end else begin
            Flush_Delay  <= Branch_Taken;  // 점프 발생을 1클럭 기억
            Flush_Delay2 <= Flush_Delay;   // 한 단 더 늦춰 2클럭 기억
        end
    end

    // Extended_Flush: 점프 결정(T) + 늦게 올라오는 구간(T+1, T+2)을 모두 덮는 차단막.
    //   이 신호가 1인 동안 IF/ID 로 들어오는 명령은 "점프로 죽어야 할 명령"이므로
    //   valid=0 으로 태깅한다 (아래 IF/ID 단계 참조).
    wire        Extended_Flush = Branch_Taken | Flush_Delay | Flush_Delay2;
    ...
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            ...
            INSTR_reg   <= INSTR;       // 매 클럭마다 메모리 출력을 녹화해둠
            Was_Stalled <= Stall_Flag;  // 방금 전 클럭이 스톨이었는지 기억함
        end
    end
    ...
    wire use_buffer = Was_Stalled & ~Extended_Flush;
    wire [31:0] safe_INSTR = use_buffer ? INSTR_reg : INSTR;
    wire [31:0] safe_PC    = use_buffer ? (PC - 32'd4) : PC;
    ...
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            ...
            // 점프 결정 + 메모리 지연으로 늦게 올라오는 구간:
            //   여기서 IF/ID 로 들어오는 명령은 "점프로 죽어야 할 명령"이다.
            //   명령어 자체는 그대로 싣되 Valid=0 으로 태깅 → ID/EX 입구에서 NOP 처리됨.
            //   (NOP 으로 즉시 비워도 되지만, Valid 태깅이 타이밍에 더 둔감하다.)
            IF_ID_PC    <= 32'd0;
            IF_ID_Inst  <= 32'd0;
            IF_ID_Valid <= 1'b0;
        end else if (Stall_Flag) begin
            // Load-Use Stall : IF/ID 유지 (같은 명령어를 한 번 더 보게 함)
            IF_ID_PC    <= IF_ID_PC;
            ...
            IF_ID_Valid <= 1'b1;     // 정상 fetch 된 명령
        end
    end
    ...
    always @(posedge CLK or negedge RSTN) begin
        if (!RSTN) begin
            ...
            ID_EX_MemToReg  <= 1'b0;
            ID_EX_ALUSrc    <= 1'b0;
        end else if (Branch_Taken || Stall_Flag) begin
